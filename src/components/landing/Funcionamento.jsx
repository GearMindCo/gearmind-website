export default function Funcionamento() {
  ...
  return (
    <section className="relative bg-black overflow-hidden -mt-35">
        ...
        {/* Cabeçalho */}
          <div className="text-center max-w-3xl mx-auto mb-10">
            ...
            <h2 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tight mb-6">
              COMO O <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-amber-300 italic">GEARMIND</span> PENSA
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 relative">
          ...
          {pipeline.map((step, index) => (
              <div className="relative group h-full">
                ...
                {/* Cartão Principal */}
                <div className="relative h-full p-8 rounded-3xl bg-white/[0.02] border border-white/10 hover:bg-white/[0.04] hover:border-orange-500/40 transition-all duration-500 flex flex-col backdrop-blur-sm overflow-hidden mt-8 lg:mt-0">
                  
                  {/* Número Gigante em Watermark */}
                  <div className="absolute -top-6 -right-4 text-8xl font-black text-white/[0.03] group-hover:text-orange-500/[0.05] transition-colors duration-500 pointer-events-none select-none">
                    {step.id}
                  </div>

                  {/* Ícone / Indicador de Passo */}
                  <div className="w-12 h-12 rounded-full bg-black border border-orange-500/30 flex items-center justify-center mb-8 relative z-10 group-hover:scale-110 group-hover:border-orange-500 transition-all duration-500 shadow-[0_0_15px_rgba(249,115,22,0)] group-hover:shadow-[0_0_20px_rgba(249,115,22,0.2)]">
                    <span className="text-orange-500 font-bold text-lg">{step.id}</span>
                  </div>
